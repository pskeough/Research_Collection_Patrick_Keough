...
| **Same Bias, Different Mechanisms** *(PsychScope)* | A counterfactual audit of socioeconomic-cue bias across three open-weight model families: an identical poverty-graded bias whose mechanism is model-idiosyncratic and mostly invisible to any single sparse dictionary, so behavioral auditing (not interpretability) must stay the load-bearing instrument. | [Repo + paper](https://github.com/pskeough/psych_scope) · preprint draft |
