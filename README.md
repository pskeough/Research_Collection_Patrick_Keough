...
| **PsychBench** | LLM-simulated psychiatric patients are individually plausible but epidemiologically wrong: +4–7 PHQ-8 points vs. NHANES ground truth in every benchmarkable group (N=28,800, 4 frontier models). | Preprint: [arXiv:2604.17359](https://arxiv.org/abs/2604.17359) · v2 (corrected ground truth + changelog) in preparation · submitted to *AI and Ethics* |
| **The Explanation Gap** | Models reproduce a large SES bias in clinical scoring (d=1.95) while their explanations conceal it — SES drives ~49% of score variance but is named least in self-explanations. | Preprint in final preparation |
| **PsychBench: Abliterated** | Removing the refusal direction from open-weight models redistributes demographic over-pathologization rather than removing it; three architecture-dependent regimes. | Preprint in final preparation |
| **PsychScope** | Mechanistic (SAE / activation-patching) dissection of the poverty-pathologization effect across three model families; the causal content largely lives in what the SAE dictionary discards. | Study executed · write-up in progress |
| **Access, Not Capability** | On private-corpus QA, a frontier API mostly sells document *access*: retrieval + a 4–8B local model closes most of the gap (judges can't separate them in 77% of pairs). | Preprint in final preparation · TMLR target |
...
| **The Listening Gap (SonicEQ)** | LLMs collapse semantic EQ descriptors ("warm", "bright") to a hyper-central curve — ~5–6× less dispersion than human engineers (SAFE-DB) — and naive RAG grounding makes it worse. | Working paper in preparation |
| **Leave the Image Alone** | Image preprocessing (binarize/upscale/downscale) monotonically degrades VLM transcription of already-legible documents; raw input wins across all four models tested. | Pilot preprint in preparation |
